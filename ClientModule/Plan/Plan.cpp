...
void InitLogger()
{
	...
	if (g_Logger->Init(lf, ll, pf))
	{
		...
		g_Logger->TryInfo(_T("Plan Start!"));
		g_Logger->TryInfo(_T("Plan %s"), valStr);
	}
	else
	{
		...
		sprintf_s(pzBuff, _T("Load LogFile Failed (%u)\n%s!"), eCode, eDescMsg);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPSTR /*lpCmdLine*/, int nCmdShow)
{
	...
	R = Plan::Resource::GetInstance();
	if (g_Util == NULL || R == NULL) return FreeMain(-2);
	...
	Plan::Resource::g_SkinResID = g_Util->ReadSkinConfig();
#ifdef SKIN_RES_TEST
	pzFilePath.Empty();
	pzResourcePath.Empty();
	pzFilePath.Format(_T("%sPlan\\skin_res\\"), CPaintManagerUI::GetInstancePath().GetData());
	if (g_Util->GetSkinResPath(pzFilePath, pzResourcePath))
	{
		...
		::StrToCharArray(Plan::Resource::g_ResourcePath, CPaintManagerUI::GetResourcePath().GetData());
		::memset(Plan::Resource::g_ResourceDllPath, 0, ArrayCount(Plan::Resource::g_ResourceDllPath) * sizeof(TCHAR));
	}
#else
	pzFilePath.Empty();
	pzResourcePath.Empty();
	pzFilePath.Format(_T("%sPlan\\skin\\"), CPaintManagerUI::GetInstancePath().GetData());
	if (g_Util->GetSkinResPath(pzFilePath, pzResourcePath))
	{
		pzResourcePath.Format(_T("%s%s"), pzFilePath.GetData(), pzResourcePath.GetData());
		if (g_Util->LoadSkinResFromDll(pzResourcePath))
		{
			bLoadResComplete = true;
			CPaintManagerUI::SetResourcePath(pzFilePath);
			CPaintManagerUI::SetResourceDll(Plan::Resource::g_ResourceInst);
			::StrToCharArray(Plan::Resource::g_ResourcePath, CPaintManagerUI::GetResourcePath().GetData());
			::StrToCharArray(Plan::Resource::g_ResourceDllPath, pzResourcePath.GetData());
		}
	}
