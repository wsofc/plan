...
typedef std::function< void(UINT_PTR idEvent, DWORD dwTime)> ProFuncTimer;

typedef struct WAPI_TIMER_PARAM
{
	HWND hWnd;
	ProFuncTimer pCallBack;
	UINT_PTR uIDEvent;
}TimerStruct;
...
public:
	...
	void SetTimer(HWND hWnd, ProFuncTimer pCallBack, UINT_PTR nIDEvent, UINT uElapse);
